...
def compute_turn_update_gigpo_advantage_return_with_discount(
    ...
    This version additionally considers the temporal structure of turns within
    trajectories when computing advantages.
    
    Args:
        values: `(torch.Tensor)`
            shape: (bs, response_length). Value estimates (added for this version).
        Other args same as compute_turn_update_gigpo_advantage_return.
    
    Returns:
        advantages: `(torch.Tensor)`
            shape: (bs, response_length). Normalized advantages.
        returns: `(torch.Tensor)`
            shape: (bs, response_length). Computed returns.
    """
    ...
    # Use utility functions to organize data
    unique_turns, duplicate_turns = identify_unique_and_duplicate_turns(env_ids, turn_ids)
    trajectories = group_turns_by_trajectory(unique_turns)
    
    # Step 1: Compute discounted returns for each turn
    turn_returns = {}
    
    for env_id, turns in trajectories.items():
        # Process turns backward to compute discounted returns
        ...
        for i in range(len(turns) - 1, -1, -1):
            ...
            # Compute total reward for this turn
            token_rewards_sum = token_level_rewards[batch_idx][valid_positions].sum().item()
        ...
        if len(group) == 1:
            ...
            if std_return > 1e-8:
                for batch_idx, ret in group:
                    normalized_advantage = (ret - mean_return) / std_return
                    normalized_advantages[batch_idx] = normalized_advantage
                    
                    # Set returns at first valid position
                    valid_positions = loss_mask[batch_idx].nonzero(as_tuple=True)[0]
                    if len(valid_positions) > 0:
                        first_pos = valid_positions[0].item()
                        returns[batch_idx, first_pos] = ret
            else:
                for batch_idx, ret in group:
                    normalized_advantages[batch_idx] = 0.0
                    valid_positions = loss_mask[batch_idx].nonzero(as_tuple=True)[0]
    ...
    # Step 4: Assign advantages to all tokens
    for batch_idx, advantage_value in normalized_advantages.items():
        valid_positions = loss_mask[batch_idx].nonzero(as_tuple=True)[0]
        for pos in valid_positions:
            advantages[batch_idx, pos] = advantage_value
