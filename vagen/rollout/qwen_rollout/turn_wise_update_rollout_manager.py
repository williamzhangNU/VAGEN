...
Each time, we only send the current obs from env to the actor and get response
We prepare a single turn of state-reponse for update
The bi-level GAE now becomes:
1. get diminished reward for each turn reward_i=reward_i+gamma*reward_i+1
    ...
    def _single_recording_to_prompt(self,
        ...
        for i, record in enumerate(history):
            if i>0:
                ...
                # Now we only update the last reponse to make it single turn style
                filtered_llm_raw_response = self._handle_special_tokens(llm_raw_response, prep_for_loss_mask=prep_for_loss_mask and i==len(history)-1)
