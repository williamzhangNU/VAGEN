...
Your response should be in the format of:
<think><observation>{{"player":(row1,column1),"box":(row2,column2),"target":(row3,column3)}}</observation><reasoning>...</reasoning><prediction>{{"player":(row1,column1),"box":(row2,column2),"target":(row3,column3)}}</prediction></think><answer>...</answer>"""
    
    if add_example:
        example = f"""e.g. <think><observation>{{"player":(2,3),"box":(4,3),"target":(5,3)}}</observation><reasoning>I need to go down then push the box down to the target</reasoning><prediction>{{"player":(4,3),"box":(5,3),"target":(5,3)}}</prediction></think><answer>Down{action_sep}Down</answer>"""
        return base_prompt + '\n' + example
    return base_prompt
