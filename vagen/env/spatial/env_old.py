...
class SpatialGym(gym.Env):
    ...
    def reset(self, seed: int = None):
        ...
        obs = self._generate_initial_observation() if not info.get('finish', False) else {"obs_str": self.prompter.task_finished_message()}
        ...
        if not action:
            ...
            if action_sequence.final_action and action_sequence.final_action.is_term():
                ...
                if self.evaluation_manager.check_and_prune_completed_tasks():
                    return {'obs_str': self.prompter.task_finished_message()}, 0, True, info, exp_log
    ...
    def _step_evaluation(self, action: str):
        ...
        return {'obs_str': self.prompter.task_finished_message()}, reward, True, {}, eval_log
